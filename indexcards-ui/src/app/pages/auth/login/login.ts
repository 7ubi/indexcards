...
export class Login implements OnInit {
  ...
  private throwInvalidForm() {
    if (!this.loginFormGroup.get('username')?.valid) {
      this.snackbar.open(
        `${this.translateService.instant(`common.error`)}: ${this.translateService.instant(`auth.username_required`)}`,
        'Ok',
        {
          duration: 3000,
        },
    ...
    if (!this.loginFormGroup.get('password')?.valid) {
      this.snackbar.open(
        `${this.translateService.instant(`common.error`)}: ${this.translateService.instant(`auth.password_required`)}`,
        'Ok',
        {
          duration: 3000,
        },
